...
func _observer_feed_text(event:String,action:String,target:String,why:String)->String:
	if event=="preference_transition": return "%s — %s" % [clock.text(),_japanese_preference_transition(why)]
	if event=="habit_transition": return "%s — %s" % [clock.text(),_japanese_habit_transition(why)]
	if event=="skill_learned": return "%s — いつものやり方を覚えた。" % clock.text()
	if event=="activity_completed":
		return "%s — %sが終わった。" % [clock.text(),ObserverText.activity_label(action).trim_suffix("している")]
	if event=="interrupted": return "%s — %sを中断した。" % [clock.text(),ObserverText.activity_label(action)]
	if event=="plan_started": return "%s — %s" % [clock.text(),why.left(120)]
	return "%s — %s" % [clock.text(),why]

func _japanese_preference_transition(why:String)->String:
	var action:=why.trim_suffix(" is becoming a preference")
	return "%sを好むようになってきた。" % ObserverText.PREFERENCE_NAMES.get(action,ObserverText.activity_label(action))

func _japanese_habit_transition(why:String)->String:
	var parts:=why.trim_suffix(".").split(" in the ")
	if parts.size()==2:return "%sに%sことが増えてきた。" % [parts[1],ObserverText.PREFERENCE_NAMES.get(parts[0],ObserverText.activity_label(parts[0]))]
	return "同じ行動を繰り返すことが増えてきた。"
...
func _update_ui() -> void:
	...
	labels["action"].text = "現在\n" + ObserverText.status(status,activity_executor.activity_id)
	labels["reason"].text = "公開された理由：" + (intention if intention!="" else reason)
	labels["connection"].text = ObserverText.connection(llm_status) + "　" + ("保存済み" if save_status!="" else "")
	...
	var text := "今の状態\n"
	for key in needs_model.values:
		var threshold:=ActivityExecutor.CRITICAL_SLEEPINESS if key=="sleepiness" else ActivityExecutor.CRITICAL_THIRST if key=="thirst" else ActivityExecutor.CRITICAL_TOILET if key=="toilet_need" else ActivityExecutor.CRITICAL_DISCOMFORT if key=="discomfort" else 100.1
		text += "%s：%s\n" % [ObserverText.need_label(key),ObserverText.need_state(float(needs_model.values[key]),threshold)]
	text += "\n部屋の状態\n清潔さ：%s\nゴミ：%s\n食料：%s\n" % [ObserverText.room_quality(room_state.cleanliness),ObserverText.resource_quality(int(room_state.resources.get("trash",0)),"trash"),ObserverText.resource_quality(room_state.item_quantity("simple_food"),"food")]
	...
	for key in pref_summary:
		var preference_text:=ObserverText.preference_text(str(key),float(pref_summary[key])); if preference_text!="": text += preference_text+"\n"
	text += "\nこの人らしさ\n" + ("まだ分からない\n" if habit_store.summary().is_empty() else "\n".join(_japanese_habits())+"\n")
	text += "\n身についたこと\n"
	if skill_store.skills.is_empty(): text += "まだない\n"
	else:
		for skill in skill_store.skills.slice(0,min(5,skill_store.skills.size())): text += ObserverText.skill_text(skill)+"\n"
	text += "\n姿勢：%s\n" % ("座っている" if resident_state.posture=="sitting" else ("横になっている" if resident_state.posture=="lying" else "立っている"))
...
func _draw() -> void:
	...
	for id in room_state.objects:
		...
		if bool(object.get("movable",false)): draw_rect(Rect2(p-Vector2(18,18),Vector2(36,36)),Color("#bcaaa4")); draw_string(ThemeDB.fallback_font,p-Vector2(14,24),ObserverText.object_label(str(id)),HORIZONTAL_ALIGNMENT_LEFT,-1,10,Color.WHITE)
	...
	if resident_state.held_item_id!="": draw_circle(render_position+Vector2(30,0),7,Color("#ffcc80")); draw_string(ThemeDB.fallback_font,render_position+Vector2(38,5),ObserverText.object_label(resident_state.held_item_id),HORIZONTAL_ALIGNMENT_LEFT,-1,11,Color.WHITE)
...
func _activity_text()->String:
	...
	if plan_executor.active:return "何かをしている"
	return "ぼんやりしている" if status=="idle" else ObserverText.status(status)

func _japanese_habits()->Array:
	var out:Array=[]
	for row in habit_store.habits:
		if float(row.get("confidence",0.0))<0.16: continue
		var activity_id:=str(row.get("activity","")); var name:String=str(ObserverText.PREFERENCE_NAMES.get(activity_id,ObserverText.activity_label(activity_id))); out.append("%sに%sことが多い" % [str(row.get("context",{}).get("time_of_day","時間帯")),name])
	return out.slice(0,min(5,out.size()))
...
func _show_settings()->void:
	var dialog:=AcceptDialog.new(); dialog.title="設定"; var box:=VBoxContainer.new(); var url:=LineEdit.new(); url.text=str(config_data.get("base_url",DEFAULT_CONFIG.base_url)); url.placeholder_text="AI接続先"
	var model:=LineEdit.new(); model.text=str(config_data.get("model",DEFAULT_CONFIG.model)); model.placeholder_text="モデル名"
	var auto:=CheckButton.new(); auto.text="自動保存"; auto.button_pressed=bool(config_data.get("auto_save",true)); box.add_child(url);box.add_child(model);box.add_child(auto);dialog.add_child(box);add_child(dialog);dialog.confirmed.connect(func(): config_data["base_url"]=url.text;config_data["model"]=model.text;config_data["auto_save"]=auto.button_pressed;var f:=FileAccess.open("user://one_room_config.json",FileAccess.WRITE);if f:f.store_string(JSON.stringify(config_data));save_status="自動保存中" if auto.button_pressed else "自動保存なし");dialog.popup_centered()

func _confirm_reset()->void:
	var dialog:=ConfirmationDialog.new(); dialog.title="新しい生活を始める？";dialog.dialog_text="現在の保存データを消去します。";add_child(dialog);dialog.confirmed.connect(func():DirAccess.remove_absolute(ProjectSettings.globalize_path(SaveManager.SAVE_PATH));get_tree().reload_current_scene());dialog.popup_centered()
