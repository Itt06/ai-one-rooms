...
const DEFINITIONS := {
	...
	"use_toilet":{"display_name":"Use toilet","target_type":"object","duration_minutes":10.0,"effects":{"discomfort":-12.0}},
	"take_shower":{"display_name":"Take shower","target_type":"object","duration_minutes":30.0,"effects":{"hygiene_need":-55.0,"discomfort":-15.0}},
	"sit":{"display_name":"Sit","target_type":"object","duration_minutes":10.0,"effects":{}},
	...
	"look_out_window":{"display_name":"Look out window","target_type":"object","duration_minutes":15.0,"effects":{"boredom":-8.0}},
	...
	for id in DEFINITIONS:
		...
		for object_id in room.objects:
			...
			if id in object.get("supported_actions", []):
				result.append(_candidate(id, definition, str(object_id)))
				matched_target = true
		if not matched_target:
			continue
	return result

static func _candidate(id: String, definition: Dictionary, target_id: String) -> Dictionary:
	return {
		"id": id,
