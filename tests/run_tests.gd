...
func _test_resident_visual_mapping() -> void:
	...
	_check(ResidentVisualAdapter.region_for("","moving","standing",0).position == Vector2(ResidentVisualAdapter.COL*2.0,ResidentVisualAdapter.ROW), "moving should use recovered walk sprite")
	_check(ResidentVisualAdapter.region_for("sleep","acting","lying",0).position == Vector2(ResidentVisualAdapter.COL,ResidentVisualAdapter.ROW*2.0), "sleep should use recovered sleep sprite")
	_check(ResidentVisualAdapter.region_for("read","acting","sitting",0).position == Vector2(ResidentVisualAdapter.COL*2.0,ResidentVisualAdapter.ROW*2.0), "read should use recovered read sprite")
	_check(ResidentVisualAdapter.region_for("use_pc","acting","sitting",0).position == Vector2(ResidentVisualAdapter.COL*3.0,ResidentVisualAdapter.ROW*2.0), "pc should use recovered pc sprite")
	_check(ResidentVisualAdapter.region_for("","idle","sitting",0).position == Vector2(0,ResidentVisualAdapter.ROW*2.0), "sitting should use recovered sit sprite")
