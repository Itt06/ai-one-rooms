...
func _test_multiday_world_dynamics() -> void:
	...
	var recovery_room:=RoomState.new(); var recovery_resident:=ResidentState.new(); recovery_resident.current_cell=recovery_room.objects.sink.interaction_cells[0]; var recovery_needs:=ResidentNeeds.new(); recovery_needs.values.thirst=97.0; recovery_needs.values.toilet_need=97.0
	var drinker:=ActivityExecutor.new(); _check(bool(drinker.begin("drink","sink","recover thirst",recovery_room,recovery_needs,recovery_resident).get("ok",false)), "critical-ish thirst should still permit sink drinking"); drinker.update(10.0); _check(bool(drinker.complete(recovery_room,recovery_needs,recovery_resident).get("ok",false)) and recovery_needs.values.thirst<90.0, "drink should leave critical thirst")
	recovery_resident.current_cell=recovery_room.objects.toilet.interaction_cells[0]; var toilet_user:=ActivityExecutor.new(); _check(bool(toilet_user.begin("use_toilet","toilet","recover toilet need",recovery_room,recovery_needs,recovery_resident).get("ok",false)), "critical-ish toilet need should permit toilet use"); toilet_user.update(10.0); _check(bool(toilet_user.complete(recovery_room,recovery_needs,recovery_resident).get("ok",false)) and recovery_needs.values.toilet_need<90.0, "toilet use should leave critical toilet need")
