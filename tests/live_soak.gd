...
func _initialize() -> void:
	...
	if fresh_run:
		scene.clock=WorldClock.new(); scene.room_state=RoomState.new(); scene.needs_model=ResidentNeeds.new(); scene.memory_store=MemoryStore.new(); scene.goal_store=GoalStore.new(); scene.preferences=PreferenceStore.new(); scene.habit_store=HabitStore.new(); scene.finance=ResidentFinance.new(); scene.relationships=RelationshipStore.new(); scene.skill_store=SkillStore.new(); scene.plan_history=PlanHistory.new(); scene.recent_activity_history=[]; scene.diary=[]; scene.decision_history=[]; scene.resident_state=ResidentState.new(); scene.resident_state.render_position=scene._cell_to_position(scene.resident_state.current_cell); scene.resident_mind=ResidentMindState.new();scene.mind_dirty=true;scene.mind_dirty_reason="fresh_soak";scene.mind_bands=scene._current_mind_bands(); scene.status="idle"; scene.reason="The room is quiet."; scene.intention=""
		scene.last_experience={}; scene.experience_pending_result={}
