...
static func validate(step, room: RoomState, grid: RoomGrid, resident: Dictionary, items: Dictionary) -> Dictionary:
	...
	var definition:Dictionary=PrimitiveToolCatalog.TOOLS[tool]
	if definition.target=="none" and not raw_args.is_empty(): return _fail("arguments_not_allowed")
	...
	if tool in ["sit","lie_down"]:
		if not room.objects.has(target) or tool=="sit" and target not in ["bed","chair"]: return _fail("invalid_posture_target")
		if not _near_any(resident.get("current_cell",Vector2i(-1,-1)),room.objects[target].get("interaction_cells",[])): return _fail("target_not_reachable")
	if tool in ["move_object","rotate_object"]:
		...
		if not _near_any(resident.get("current_cell",Vector2i(-1,-1)),room.objects[target].get("interaction_cells",[])): return _fail("target_not_reachable")
		if tool=="move_object":
			var placement:=room.validate_object_placement(target,Vector2i(int(raw_args.x),int(raw_args.y)),int(room.objects[target].get("rotation",0)),[resident.get("current_cell",Vector2i(-1,-1))])
