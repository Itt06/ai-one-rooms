...
const TOOLS := {
	...
	"remote_work":{"target":"object","args_schema":{"target":"object_id"}},"masturbate":{"target":"none","args_schema":{}},"message_contact":{"target":"contact","args_schema":{"target":"contact_id"}},"call_contact":{"target":"contact","args_schema":{"target":"contact_id"}},"meet_contact":{"target":"contact","args_schema":{"target":"contact_id"}},"invite_for_sex":{"target":"contact","args_schema":{"target":"contact_id"}},"sex":{"target":"contact","args_schema":{"target":"contact_id"}}
}

static func available(room:RoomState,resident:Dictionary,context:Dictionary={})->Array:
	var result:Array=[]
	for id in TOOLS:
		...
		if id=="order_groceries" and int(context.get("cash",999999))<ResidentFinance.GROCERIES_COST:continue
		var targets:Array=[]
		if TOOLS[id].target=="object":targets=_object_targets(room,id)
		elif TOOLS[id].target=="item":targets=_item_targets(room,id)
		elif TOOLS[id].target=="contact":targets=context.get("contact_targets",[])
		if id=="invite_for_sex":targets=context.get("sexual_partner_options",[])
		if id=="sex":targets=context.get("accepted_partner_targets",[])
		...
		if TOOLS[id].target in ["object","item"]:entry["target_hints"]=_target_hints(room,id,targets,resident)
		result.append(entry)
	return result
...
static func _object_targets(room:RoomState,tool:String)->Array:
	...
	for id in room.objects:
		...
		if tool in ["use_pc","order_groceries","remote_work"] and id=="pc" and not bool(object.get("state",false)):continue
